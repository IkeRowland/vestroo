...
| Runs | **`service_runs`** | Aligned. Optional **`chauffeur_id`** on run row. |
...
| Close protection | **`close_protection_engagements`** (VST-11) — **`booking_id`** (required), **`trip_id`** (nullable), **`status`**, **`coordination_notes`**, **`created_by`** → **`profiles`** | **MVP:** staff-only; **VST-12** retention/export hooks (**[compliance-and-safety.md](compliance-and-safety.md)**). |
...
| **`booking_metadata`** | JSON bag. For **`experience_package`**: **`experience_package_id`** (uuid), **`experience_date`** (ISO string), **`group_size`**, **`selected_addon_ids`** (string[]). See **[tours-and-experiences.md](tours-and-experiences.md)**. **Close protection (VST-11):** optional **`close_protection_requested`** / **`close_protection_engagement_id`** documented in **[close-protection-engagements.md](close-protection-engagements.md)** — **no** new **`booking_intent`** value; engagements live in **`close_protection_engagements`**. |
...
| **`paid`** | **`paid`** | PayFast webhook **`COMPLETE`**. |
...
- **Customer / passenger:** **`bookings`** (own `customer_id`), **`tickets`** (own `passenger_id`), **`trips`** as customer or chauffeur party, **`booking_trips`** via booking ownership, **`notifications`** as recipient, **`ratings`** as party.
- **Chauffeur:** Row access via **`chauffeur_id = auth.uid()`** on trips, conversations, ratings, shared itineraries; **`chauffeur_assignments`** / **`chauffeur_schedules`** self rows; **`vehicle_trackings`** when linked to own assignment. **VST-8:** additional **`bookings`** / **`booking_trips`** **select** policies when the booking is linked to a trip owned by the chauffeur (guest booking PII for active legs only — see migration **`20260408120000_*`**).
...
**`close_protection_engagements` (VST-11):** RLS **enabled**; **no** policies for **`anon`** — deny-all. **`authenticated`:** **only** **`is_staff(auth.uid())`** may **select / insert / update / delete** (named policies in **`20260411120000_vst11_close_protection_engagements.sql`**; **insert** requires **`created_by = auth.uid()`**). Customers and chauffeurs have **no** access.
...
Policy definitions are split across historical **`vin_shuttle_rls_*.sql`** (enable + early policies, some superseded), **`20260402133618_vestroo_drop_legacy_rls_policies.sql`**, **`20260402133646_vestroo_rls_policies_vestroo_domain.sql`**, **`20260402133655_vestroo_rls_policies_booking_social.sql`**, **`20260402133703_vestroo_rls_policies_tracking_drivers.sql`**, **`20260406103000_vestroo_profile_roles_chauffeur_columns_rls.sql`** (VST-5 role + column + policy refresh), **`20260408120000_vst8_chauffeur_booking_rls_ops_audit_actor_role.sql`** (VST-8 chauffeur booking visibility + audit **`actor_role`**), **`20260411120000_vst11_close_protection_engagements.sql`** (VST-11), and **`20260412120000_vst12_compliance_incidents_documents_retention.sql`** (VST-12).
...
| **`action`** | Staff: e.g. `assign_booking_to_run`, `update_trip_status`, `record_trip_delay`, `swap_trip_vehicle`, `create_close_protection_engagement`, `update_close_protection_engagement`, `create_compliance_incident`, `create_vehicle_compliance_document`, `create_chauffeur_compliance_document`. **Admin:** `dsr_export`, `dsr_anonymise`. Chauffeur: `chauffeur_*` as above. |
...
- **Emission:** Server Actions **`opsDispatch`**, **`fieldChauffeur`**, and **`publishChauffeurLocationAction`** do not replace email/push; see **`docs/realtime-and-notifications.md`**.
...
### Close protection inheritance (VST-11 + VST-12)

**`close_protection_engagements`** remain defined by **[close-protection-engagements.md](close-protection-engagements.md)**. For DSR anonymise, coordination notes on engagements tied to affected bookings are cleared; **status** semantics are **unchanged** here.
