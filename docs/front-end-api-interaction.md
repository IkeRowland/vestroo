...
| `processPayment` | Same reconcile + insert + PayFast signature; **`amount`** and DB **`total_amount`** use reconciled ZAR only. Returns **`payfastProcessBaseUrl`** (from server env **`PAYFAST_URL`**) for the client form `action` — no **`NEXT_PUBLIC_PAYFAST_*`**. Optional corporate fields (**`invoiceRequested`**, **`purchaseOrderRef`**, **`billingEntityRef`**) persist to **`bookings`** columns. |
...
| `createCloseProtectionEngagementAction` | **Staff JWT** — Zod **`bookingId`** (+ optional **`status`**, **`coordinationNotes`**); inserts **`close_protection_engagements`** with **`created_by = auth.uid()`**; **`ops_audit_log`** **`create_close_protection_engagement`**. |
| `updateCloseProtectionEngagementAction` | **Staff JWT** — Zod **`engagementId`** + optional **`status`**, **`coordinationNotes`**, **`tripId`** (must be linked via **`booking_trips`** when set); audit **`update_close_protection_engagement`** (payload: ids + **`fields_changed`** only). |
| `listCloseProtectionEngagementsAction` | **Staff JWT** — Zod optional **`limit`**, **`bookingId`**, **`tripId`**; returns recent rows for **`/ops/close-protection`**. |
| `getCloseProtectionEngagementByIdAction` | **Staff JWT** — Zod **`engagementId`**; single row for detail page. |
| `getCloseProtectionEngagementsByBookingIdAction` | **Staff JWT** — Zod **`bookingId`**; all engagements for that booking. |
...
| `exportDataSubjectAction` | **Admin JWT only** (`getOpsAdminForAction` — **`ProfileRole === 'admin'`**, not merely **`is_staff`**) — Zod **`profileId`** and/or **`email`**; **`profiles.role`** must be **`customer`**. Returns minimal JSON **`vst12_dsr_minimal_v1`**: profile subset, deduped **bookings** (by **`customer_id`** or guest **`customer_email`** match), **trips** with **`customer_id`**, **CP engagements** (ids/status/timestamps only — no **`coordination_notes`**). **`ops_audit_log`** **`dsr_export`**. Uses **`createUserServerClient()`** (staff RLS sufficient). |
| `anonymiseDataSubjectAction` | **Admin JWT only** — Zod **`profileId`** + confirm literal **`ANONYMISE`**; customer profiles only; redacts **profile** PII, **booking** guest contact fields, clears **CP** **`coordination_notes`** for linked bookings, nulls **`trips.customer_id`**; audit **`dsr_anonymise`** with **`auth_users_followup_required`**. |
...
**Dispatch:** **`assignBookingToRun`** also sets **`close_protection_engagements.trip_id`** for rows with matching **`booking_id`** and null **`trip_id`** when a trip is created (VST-11).
...
| `POST /api/payfast/webhook` | **`verifyPayFastWebhookSignature`** only (`src/lib/payfast.ts`) → idempotent **`COMPLETE`** (conditional update; **no** duplicate confirmation emails on retries; **`200`** for benign duplicates) → **`FAILED`/`CANCELLED`** sets **`payment_status = failed`**, **`status = pending`** without downgrading **paid** rows. See **[integrations-and-payments.md](integrations-and-payments.md)**. |
