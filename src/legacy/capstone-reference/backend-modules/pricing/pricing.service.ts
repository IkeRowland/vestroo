...
export class PricingService implements IPricingService {
  ...
  async createBusRoutePricing(dto: ICreateBusRoutePricingDto) {
    ...
    let lineHaulServiceConfig = await this.configRepo.findByServiceType(ServiceType.SCHEDULED_CORPORATE_ROUTE);
    if (!lineHaulServiceConfig) {
      lineHaulServiceConfig = await this.configRepo.create({
    ...
    const existingPricing = await this.vehiclePricingRepo.findVehiclePricing({
      ...
      service_config: lineHaulServiceConfig._id,
    });
    ...
    const newPricing = await this.vehiclePricingRepo.create({
      vehicle_category: dto.vehicle_category,
      service_config: lineHaulServiceConfig._id.toString(),
  ...
  async calculateBusFare(
    ...
    const lineHaulServiceConfig = await this.configRepo.findByServiceType(ServiceType.SCHEDULED_CORPORATE_ROUTE);
    if (!lineHaulServiceConfig) {
      throw new HttpException(
        {
          statusCode: HttpStatus.NOT_FOUND,
    ...
    const result = await this.recipePrice(
      lineHaulServiceConfig.base_unit,
