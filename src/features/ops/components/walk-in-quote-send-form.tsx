...
import { sendWalkInQuote } from '@/actions/sendWalkInQuote'
...
type WalkInQuoteSendFormProps = {
	bookingId: string
	defaultFirstLineLabel: string
}

export function WalkInQuoteSendForm({ bookingId, defaultFirstLineLabel }: WalkInQuoteSendFormProps) {
	...
	return (
		<div className="mt-4 space-y-4">
			<p className="text-xs text-ops-muted">
				Enter one or more line items (description and unit price in ZAR). Line total is qty × unit
				price. The email uses this breakdown.
			</p>
							...
							<tr key={r.key} className="border-b border-ops-border/80 last:border-0">
								<td className="px-2 py-2">
									<Input
										...
										disabled={pending}
										placeholder="e.g. Transfer — CPT to Stellenbosch"
			...
			<Button
				...
				onClick={(e) => {
					...
					startTransition(async () => {
						const res = await sendWalkInQuote({ bookingId, lineItems })
						if (res.ok) {
							router.refresh()
							return
						}
