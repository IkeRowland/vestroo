 ...
 */
function isTimeInRange(time: string, startTime: string | null, endTime: string | null): boolean {
 ...
 */
async function getMatchingPricingRules(
  ...
  for (const rule of rulesResult.docs) {
    ...
    if (rule.route_id && routeId) {
      const routeIdValue = typeof rule.route_id === 'object' ? String(rule.route_id.id) : String(rule.route_id)
      if (routeIdValue !== routeId) {
        continue
      }
    }
    ...
    if (rule.vehicle_type_id) {
      const vehicleTypeIdValue = typeof rule.vehicle_type_id === 'object' ? String(rule.vehicle_type_id.id) : String(rule.vehicle_type_id)
      if (vehicleTypeIdValue !== vehicleTypeId) {
        continue
      }
    }

    // Check day of week match
    if (rule.day_of_week && rule.day_of_week !== 'all' && rule.day_of_week !== dayOfWeek) {
      continue
    }

    // Check time range match
    if (!isTimeInRange(pickupTime, rule.start_time || null, rule.end_time || null)) {
      continue
    }

    // Check date range match
    if (!isDateInRange(pickupDatetime, rule.start_date || null, rule.end_date || null)) {
      continue
    }
...
export async function calculatePrice(
  ...
  // Distance-based pricing (Uber-style)
  if (input.distance_km !== undefined && input.distance_km !== null) {
    const basePricePerKm = input.base_price_per_km || 15 // Default R15 per km if not provided
    basePrice = input.distance_km * basePricePerKm
  } 
  // Route-based pricing (legacy)
    ...
    if (!route || !route.base_price) {
      throw new Error(`Route with ID ${input.route_id} not found or has no base price`)
    }
  ...
  // Fetch vehicle type price multiplier
  const vehicleType = await payload.findByID({
    collection: 'vehicle-types',
    id: input.vehicle_type_id,
  })

  if (!vehicleType || !vehicleType.price_multiplier) {
    throw new Error(`Vehicle type with ID ${input.vehicle_type_id} not found or has no price multiplier`)
  }

  if (!vehicleType.is_active) {
    throw new Error(`Vehicle type with ID ${input.vehicle_type_id} is not active`)
  }

  // Apply vehicle type multiplier to base price
  const vehicleBasePrice = basePrice * vehicleType.price_multiplier

  // Get matching pricing rules
  const matchingRules = await getMatchingPricingRules(
    payload,
    input.route_id || null,
  ...
  const { finalPrice, appliedRules } = applyPriceModifiers(vehicleBasePrice, matchingRules)
